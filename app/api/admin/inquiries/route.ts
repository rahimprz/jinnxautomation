...
export async function GET(req:Request){try{
 ...
 if(exportCsv){const list=[['Name','Email','Project','Estimate USD','Status','Notes','Received UTC'],...rows.map(r=>[r.name,r.email,r.idea,r.estimate,r.status,r.notes,new Date(r.created_at).toISOString()])];return new Response('﻿'+list.map(r=>r.map(csvCell).join(',')).join('\r\n'),{headers:{'Content-Type':'text/csv; charset=utf-8','Content-Disposition':'attachment; filename="Jinnx-Automation-inquiries.csv"','Cache-Control':'private, no-store','X-Content-Type-Options':'nosniff'}})}
